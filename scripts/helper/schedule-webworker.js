...
function findSpeakerById(id, speakerList) {
  return speakerList.filter(function(s) {
    return s.id === +id
  })[0]
}
...
function attachSessionAndSpeakersTogether(session, speakersRaw) {
  ...
  if (isDefined(session.speakers)) {
    for (
      var speakerIdx = 0;
      speakerIdx < session.speakers.length;
      speakerIdx++
    ) {
      if (!isDefined(session.speakers[speakerIdx].id)) {
        session.speakers[speakerIdx] = findSpeakerById(
          session.speakers[speakerIdx],
          speakersRaw
        )
...
function hasSpeakersAndSessionsAndSchedule(speakers, sessions, schedule) {
  return (
    hasSpeakersAndSessions(speakers, sessions) && Object.keys(schedule).length
  )
}
...
self.addEventListener(
  ...
  function(e) {
    ...
    if (hasSpeakersAndSessionsAndSchedule(speakers, sessions, schedule)) {
      schedule.tags = []
      for (
        var dayIdx = 0, scheduleLen = schedule.length;
        dayIdx < scheduleLen;
        dayIdx++
      ) {
        ...
        day.tags = []
        for (
          var timeSlotIdx = 0, timeslotsLen = day.timeslots.length;
          timeSlotIdx < timeslotsLen;
          timeSlotIdx++
        ) {
          var timeslot = day.timeslots[timeSlotIdx]
          for (
            var sessionIndex = 0, sessionsLen = timeslot.sessions.length;
            sessionIndex < sessionsLen;
            sessionIndex++
          ) {
            for (
              var subSessIdx = 0,
                subSessionsLen = timeslot.sessions[sessionIndex].length;
              subSessIdx < subSessionsLen;
              subSessIdx++
            ) {
              var session =
                sessions[timeslot.sessions[sessionIndex][subSessIdx]]
              ...
              schedule[dayIdx].timeslots[timeSlotIdx].sessions[sessionIndex][
                subSessIdx
              ] = session
            }
          }
    ...
    self.postMessage({
      ...
      sessions: sessions,
      schedule:
        Array.isArray(schedule) && Object.keys(sessions).length ? schedule : []
    })
  },
  false
)
